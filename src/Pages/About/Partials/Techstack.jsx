...
function Techstack({ heading, icons }) {
  return (
    <div className="px-4 py-4 w-[90%] mx-auto text-center">
      <motion.h2
        className="text-3xl md:text-4xl font-semibold text-(--text-color) mb-10"
      ...
      >
        {icons.map((Icon, index) => (
          <motion.div
            key={index}
            className="w-24 h-24  border border-[#b562d6] rounded-lg shadow-md flex items-center justify-center text-(--text-color) text-4xl transition-transform duration-300 hover:scale-105 hover:shadow-[0_0_15px_#b562d6]"
